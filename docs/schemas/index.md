...
members ◄──────────── missionSubmissions ──────────► missions
   ...
   └──── posts, activities, applications(합격 후)
...
| 역할 | MemberRole 값 | 주요 permissions |
|---|---|---|
| 게스트 | (비로그인) | 공개 멤버·블로그·활동 읽기 |
| 멤버 | `'멤버'` | 미션 제출, 블로그 쓰기, 활동 올리기 |
| 리뷰어 | `'리뷰어'` | + `mission:review` |
| 리드/메인테이너 | `'리드'`, `'메인테이너'` | + `mission:create`, `study:manage` |
| 운영진(staff) | `'리드'` + 개별 권한 | + `recruit:review`, `admin:roles` |
| OB | `'OB'` | 멤버와 동일 (프로필에 OB 뱃지) |
