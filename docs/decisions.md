...
- **2026-07-06 (2)** 2기 확장(D19대로 상수만 편집). 2·3기 공용 카탈로그·기수 토글 추가. 실데이터 검증: 2기 FE 전원 완주, BE 완주(황혜림 머지대기 1건, 특이 예외 없음). 남해윤=미션 수행 1기라 2기 명부 제외(1기로). PR 링크에 단계 라벨 표시(각 PR이 무슨 단계인지) UI 추가.
- **2026-07-06 (3)** 닫힌(closed) PR = 재제출·실수 → 표시·집계 제외(`닫힌 N건 숨김`만 표기). 강동현·박찬빈 매핑불명(실수 close), 윤재홍 TanStack #54 닫힘(→#56 머지 재제출) 등 노이즈 해소. 완주 숫자 불변. 명부 멤버 매핑불명 0건 확인(§5.3). 1기 대비: BE 제목 `[N주차]` 파서 규칙 추가 필요 발견(§7).
- **2026-07-06 (4)** BE 미션 누락 발견·수정: 실제 3개 아닌 **5개 레포**(사다리 `java-ladder-func`, 방탈출① `spring-roomescape`=JDBC라운드 추가). 방탈출이 2라운드(JDBC 6~9주 / JPA 11~14주)임 확인. BE 12→**16단계**. 추적 레포 2개 추가·재sync(1622 PR). 2·3기 공용. 검증: 하수한·김하늘·2기 4명 16/16 완주, 김태우 방탈출② MVC 누락.
- **2026-07-06 (5)** 미션 셀 UI: gap 상태(🔴)일 때 원인이 되는 개별 단계 뱃지도 빨강으로 구분 표시(기존엔 미션 스트립만 빨강, 펼친 상세의 단계 뱃지는 회색이라 원인 불명확했음).
- **2026-07-06 (12)** 명부 정정: 신혁수·방재경·박예은·배강현 **4명이 그리디 멤버가 아님으로 확인** → `members.ts`에서 완전 제거(1기 FE 6명→2명, 송혜정·김준수만 남음, 둘 다 14/14 완주 유지). 이전엔 "PR 0건 미확인"(신혁수·방재경)·"탈퇴 멤버"(박예은·배강현)로 QA 대기 중이었으나 확인 결과 소속 자체가 아니었음. 이들의 실PR은 이제 "명부밖"으로 자동 흡수. 전 기수 진짜 누락 6건→**4건**(탈퇴 2명 케이스 소거). §7·§8 갱신, "4기 남음" 표기 정정("1·2·3·4기 완료").
- **2026-07-06 (11)** 진행 중 기수 프로세스 확정(§5.4). **프론티어(마지막 활동 미션)만** 🔷남색 "진행 중". 그 앞의 밀린 미머지는 🟡노랑 유지(정리 대상), 뒤는 ⚪미착수. (처음엔 모든 pending을 남색으로 했다가 "밀린 미머지는 진행 중 아님" 피드백으로 프론티어 한정으로 정정.) 프론티어 = cells에서 state≠none인 마지막 index로 데이터 추정. "PR 누락 N건" 플래그 숨김, 완주 판정 종료 후 보류. 중도 하차 → `member.abandoned`(미션 포기) 플래그(이태규 4기), withdrawn과 구분.
- **2026-07-06 (10)** 불참(부정행위) 처리 = override의 첫 실사용. GitHub `closed`엔 이유가 없어 "닫힘(제출)"로 오분류되던 케이스 중, AI 사용 불참(천동현 styled #59)을 `prOverrides.exclude`+note로 확정. buildCell이 제외 PR을 카운트에서 빼고 이유를 `excluded[]`로 반환 → UI에 `🚫 불참 처리: <이유>` 노출(silent 제외 아님). QA로 전 기수 "닫힘=제출" 9건 검토(1기 학기말 8 genuine + 천동현 1 불참). §5.3.
- **2026-07-06 (9)** 4기 확장 완료(진행 중 기수). FE 커리큘럼 상이 → `cohort4FE()` 분리(숫자야구 1주·룰렛·좀비·무엇이든·포켓몬SSR). BE는 1~3기와 동일 5레포. 카탈로그를 sharedFE123/sharedBE/cohort4FE로 리팩터. 파서: 룰렛·좀비(단일)·whatever·pokemon(잠정, 착수전) 추가 + spring-roomescape 단계번호 폴백(JDBC 오라벨 대응). UI: ongoing 기수 안내 배너. 검증: FE 4명 진행중(매핑불명 0), BE 6명 진행중. Search API 오탐 재현(Cappucciyes)—sync로 정정.
- **2026-07-06 (8)** 자동화 전환 준비: 하드코딩 경계(명부·카탈로그·파서·오버라이드 = 사람 큐레이션 / PR = 자동 / 집계 = 순수계산) 명문화 + 트러블슈팅 13개 함정을 자동화 주의점으로 정리(missions-dashboard §10). 핵심 결론: 5기+ 완전자동화의 갈림길은 코드가 아니라 **입력 표준화**(PR 제목 템플릿·Team 태깅·셀프등록·회원전용 레포).
- **2026-07-06 (7)** 닫힌 PR 처리 정교화(1기 재조사 계기): closed를 무조건 숨기던 걸 **재제출(이미 머지/미머지 단계 덮음 → 숨김) vs 진짜 제출(그 단계 유일 PR → `closed` 상태로 표시)**로 이분. 단계 상태에 `closed` 추가(제출·미머지, "누락"과 구분). 결과: 1기 BE 가짜 누락 대거 해소, 전 기수 진짜 누락 6건으로 수렴(김태우·탈퇴2명·1기 방탈출②후반 3명). 완주 숫자 불변. 3기 김태우 gap 회귀 없음.
- **2026-07-06 (6)** 1기 확장 완료. FE 확정 2명(송혜정·김준수, 14/14 완주) · BE 확정 7명(신지훈·안금서·황승준·정상희·김의진·신혜빈·남해윤, 12~14/16). 발견사항: ①야구 레포 삭제 안 됨(노션 메모 오래된 추정) ②racingcar·lotto가 "전 단계 통짜 제출" 방식(주차 표기)이라 `matchUnits`에 폴백 추가 ③advanced 일부 "2(1)" 괄호 표기 발견, 정규식 추가 ④방탈출②(마지막 미션)가 학기종료(2025-02)와 겹쳐 BE 전원 미완주 — 파서 버그 아닌 실제 데이터(자연 중단) ⑤ChangwooJ는 노션에 1기로도 기재됐으나 실PR은 전부 2기뿐 → 1기 이력 제거 ⑥탈퇴 멤버(ye6194·bae-kh) 실PR 이력 있어 withdrawn:true로 유지 ⑦황승준·김의진·정상희를 Search API 인덱싱 누락으로 "미확인" 오판했다가 전체 동기화 데이터 재검증으로 정정 ⑧신혁수·방재경은 이중 교차검증 후에도 PR 0건(미참여 추정). **검증 원칙 확립**: Search API 단독 신뢰 금지, 동기화 데이터로 재확인 필수(§7).
