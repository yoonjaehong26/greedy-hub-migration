...
- **파일럿 = 3기(2025-2)**. 확장 순서: **3·2기(완료) → 4기 → 1기**.
...
### 5.3 닫힌(closed) PR 처리
GitHub상 머지된 PR은 `merged`로 따로 잡히므로, **`closed` = 재제출·실수·폐기**(초반에 잘못 올렸다가 닫음 등)다. 따라서:
- 단계 상태·완주 집계에 **반영 안 함** (원래 merged/open만 셈).
- PR 목록 **표시에서 제외**하고, 미션별로 `닫힌 PR N건 숨김`만 표기(감사용).
- 재제출되어 머지된 PR이 같은 단계를 커버하므로 완주는 정상 반영됨.
...
- **1기**: ⚠️ 야구 레포(`javascript-baseball-precourse`) **1기 버전 삭제됨**. 탈퇴 멤버(박예은 `ye6194`, 배강현 `bae-kh`)는 `withdrawn:true`. spa-routing·todo-list 대신 다른 초기 레포 가능성 → 레포 재확인 필요.
  - **BE 제목 형식 차이**: 1기 BE 미션 제목이 `[N주차]`(단계 아님) → `matchUnits`의 racingcar/lotto 규칙에 **주차 인식 추가** 필요 (검증: developowl `#56 [1주차] 자동차경주`, `#68 [2주차] 로또`).
  - **1기 BE 편입 대기**: 남해윤(haeyoon1), 신지훈(developowl 1기 BE) 등. members.ts 하단 참고 주석 참조.
...
### 2기 (특이 예외 없음)
- **완주**: FE 6명 전원(14/14) · BE 5명(12/12)
- **🟡 머지만 하면 됨**: 황혜림 방탈출 Core 배포 7-9(#176) — 2기 유일한 미완
- **🔴 진짜 누락**: 없음
- **cross-cohort 플래그(정상)**: `developowl`(신지훈) 1기 BE→2기 FE 전환 · `ChangwooJ`(정창우) 1·2기 FE · `mintcoke123`(강동현) 2기 FE / 3기 BE.
- **명부 정리**: 남해윤(haeyoon1)은 2기 줍줍 데모팀 명단엔 있으나 미션 수행은 1기 창 → **2기 명부에서 제외**(1기 확장 시 편입).
