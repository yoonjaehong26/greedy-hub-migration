...
- **파일럿 = 3기(2025-2)**. 확장 순서: **3·2·1·4기 전부 완료** (5기+는 §10 자동화 가이드로).
...
> **⏳ 진행 중 기수(`COHORTS[c].ongoing`)에선 `gap`을 빨강으로 안 띄운다.** 아직 안 온 뒤쪽 미션의 "PR 없음"은 "미제출"이 아니라 "예정"이므로, `gap` 셀·단계를 **남색 "진행 중"** 으로 렌더(데이터는 그대로, 표시만 다름). "PR 누락 N건" 멤버 플래그도 진행 중엔 숨김. **완주/누락 판정은 기수 종료 후.** 상세는 §5.4.
...
### 5.4 진행 중 기수 프로세스
종료 기수와 진행 중 기수는 "미완"의 의미가 다르다. 핵심은 **"현재 하는 미션"(프론티어)만 진행 중으로 관용**하고, 그 앞의 밀린 미머지는 그대로 노랑으로 남기는 것.

**프론티어(frontier) = 그 멤버가 마지막으로 활동(제출)한 미션** (`cells`에서 `state !== 'none'`인 마지막 index). 진행 중 기수에서 **이 미션만** 남색 처리한다.

| 셀 위치·상태 | 종료 기수 | 진행 중 기수(`ongoing:true`) |
|---|---|---|
| 프론티어 미션이 `pending`/`gap` | (해당 없음) | 🔷 **진행 중**(남색) — 지금 하는 미션 |
| 프론티어 **앞** 미션이 `pending`(미머지) | 🟡 머지만 하면 됨 | 🟡 **미머지(밀림)** — 노랑 유지 |
| 프론티어 **앞** 미션이 `gap` | 🔴 PR 누락 | 🔴 PR 누락 (실제 뒤로 밀린 구멍) |
| 프론티어 **뒤** 미션 | — | ⚪ 미착수/예정 (활동 없음) |
| "PR 누락 N건" 플래그 | 표시 | **숨김** |
| 완주/미제출 판정 | 확정 | **종료 후로 보류** |

- **왜 프론티어만?** 김민욱(4기)처럼 자동차·로또(초반)가 미머지면 그건 "진행 중"이 아니라 "밀려서 아직 머지 안 됨" → 노랑으로 남겨 정리 대상 표시. 반면 방탈출②(그의 프론티어)는 지금 하는 중 → 남색.
- **색 일관성**: 프론티어 미션은 **셀뿐 아니라 펼침 상세의 미머지 뱃지·PR도 남색**(진행 중). 밀린 미션의 미머지는 상세에서도 노랑 → 색만으로 "지금 하는 것(남색)" vs "밀려서 정리할 것(노랑)"이 갈림. 라벨 텍스트는 둘 다 "미머지"라 일괄 정리 시 식별 가능.
- 시스템은 "지금 몇 주차인지"를 모르므로 프론티어를 **데이터(마지막 활동 미션)로 추정**. 자동 판정이 아니라 사람이 보는 현황판.
- **개별 판단 예외**(중도 하차)는 자동 추론 불가 → 멤버 플래그: `member.abandoned=true` → "미션 포기"(예: 4기 이태규). `withdrawn`(동아리 탈퇴)과 구분.
- **매핑불명·불참(🚫)은 진행 중에도 그대로 표시**. "진행 중" 완화는 프론티어 셀 색에만 적용.
...
## 7. 기수 확장 플레이북 (1·2·3·4기 완료 · 5기+ 예정)
...
### 1기 (예외 최다 — 완료)
- **FE 완주**: 송혜정(Songhyejeong)·김준수(gogo1414) 전원 **14/14**. (명부는 이 2명뿐 — 아래 참조)
...
- **명부 정정(2026-07-06)**: 신혁수(sins051301)·방재경(Jae-kyoung)·박예은(ye6194)·배강현(bae-kh) **4명은 그리디 멤버가 아님으로 확인** → 명부에서 제외. (이전엔 "PR 0건 미확인"·"탈퇴 멤버"로 QA 대기 중이었음.) 이들의 GitHub PR 이력은 이제 "명부밖" 카운트로만 집계.

> 전 기수(1·2·3기 종료) 🔴 진짜 누락 총 4건(명부 정정 후): 김태우(3기 방탈출②MVC) · 황승준·김의진·남해윤(1기 방탈출②JPA·Core).

### 4기 (진행 중 — 완주 판정 보류)
- **FE (17단계, ~10주차)**: 4명 전원 숫자야구·룰렛·좀비·React기초 완주, 심화 TanStack 미머지(9주차 방금), 무엇이든·포켓몬 미착수(⚪, 정상). 매핑불명 0.
- **BE (16단계)**: 정명준·김하은 14/16, 강대현·이채현 13/16, 김민욱 7/16(자동차·로또·사다리 전부 미머지 — 리뷰 대기). 방탈출②(round2) JPA·Core는 대부분 아직 진행 중(§5.4로 남색 "진행 중" 표시, 빨강 아님).
- **이태규(Cappucciyes)**: 미션 포기(중도 하차) → `abandoned:true`, "미션 포기" 플래그.
- ⏳ **주의**: 진행 중 기수라 뒤쪽 미션의 ⚪/🔷는 "실패"가 아니라 "아직 안 함". 종료 후 재판정(§5.4).
